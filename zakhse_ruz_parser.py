...
def building_of_auditory(auditory_id):
    ...
    list_of_auditories = json.loads(a.text)
    for i in range(len(list_of_auditories)):
        if list_of_auditories[i]['auditoriumOid'] == auditory_id:
            return "Building ID: " + str(list_of_auditories[i]['buildingOid']) + "; Name: " + \
                   list_of_auditories[i]["building"]
    return ""
